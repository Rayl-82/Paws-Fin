    ...
    async function fetchBundle() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error("Failed to load bundle details");
        }
    ...
    return (
      <div className="min-h-screen bg-[#F7F9FC] flex flex-col">
        ...
        <div className="flex-grow flex items-center justify-center">
          <Loader2 className="w-8 h-8 animate-spin text-[#1B6CA8]" />
        </div>
        <Footer />
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-[#F7F9FC] flex flex-col">
        ...
        <div className="flex-grow flex flex-col items-center justify-center p-4">
          ...
          <h1 className="text-2xl font-serif font-bold text-[#1A1A1A] mb-2">Bundle Tidak Ditemukan</h1>
          <p className="text-[#546E7A] mb-6">{error || "Bundle yang Anda cari tidak tersedia."}</p>
      ...
      <main className="max-w-[1280px] mx-auto px-4 md:px-8 pb-16 md:pb-24 pt-4 md:pt-8">
        ...
        <section className="flex flex-col lg:flex-row gap-8 lg:gap-16 mb-16 md:mb-24">
          ...
          <div className="flex-1 flex flex-col gap-4">
            ...
            {/* Gallery placeholder - hide if only 1 image */}
          </div>
          ...
          <div className="flex-1 flex flex-col">
            <div className="mb-6">
              <div className="inline-flex px-4 py-1.5 bg-[#F26641] text-white text-[10px] md:text-xs font-bold uppercase tracking-widest rounded-full mb-3 md:mb-4">
                {product.billingInterval || "Paket Bundle Spesial"}
              </div>
            ...
            <div className="mb-6">
              ...
              <p className="text-[#546E7A] mt-2">Termasuk berbagai macam produk pilihan yang dikurasi khusus.</p>
            </div>

            {/* Features (if any) */}
            {product.features && product.features.length > 0 && (
              <div className="mb-8">
                <h3 className="font-semibold text-[#1A1A1A] mb-4">Manfaat Utama:</h3>
                <ul className="flex flex-col gap-3">
                  {product.features.map((feature, index) => (
                    <li key={index} className="flex items-center gap-3">
                      <div className="w-6 h-6 rounded-full bg-[#E5F0F9] flex items-center justify-center flex-shrink-0">
                        <Check className="w-4 h-4 text-[#1B6CA8]" strokeWidth={3} />
                      </div>
            ...
            <div className="flex flex-col gap-4">
              <button 
                ...
                disabled={isAdding}
                className="w-full bg-[#1B6CA8] hover:bg-[#124E7A] disabled:opacity-50 text-white font-bold py-4 rounded-xl transition-colors shadow-md flex items-center justify-center gap-2"
              >
                {isAdding ? <Loader2 className="w-5 h-5 animate-spin" /> : null}
                {isAdding ? "Menambahkan..." : "Beli Bundle Sekarang"}
              </button>
              {cartSuccess && (
